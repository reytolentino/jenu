...
class Web2Market_Report_Block_Adminhtml_Filter_Form
    extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        $htmlIdPrefix   = 'queryreport_';
        $form->setHtmlIdPrefix($htmlIdPrefix);

        // create a fieldset to add elements to
        $fieldset       = $form->addFieldset('base_fieldset', array('legend' => Mage::helper('queryreport')->__('RUN OR ALTER EXISTING REPORT')));
	 $fieldset1       = $form->addFieldset('base_fieldset1', array('legend' => Mage::helper('queryreport')->__('SQL CODE BOX AND ACTION FOR EXISTING OR ORDERS')));
	 
        // prepare our filter fields and add each to the fieldset

        // date filter
        $dateFormatIso  = Mage::app()->getLocale()->getDateFormat(Mage_Core_Model_Locale::FORMAT_TYPE_SHORT);
    $adminurl=Mage::helper('adminhtml')->getUrl('/report/textfield')."?queryreport_report_type=";
		 $fieldset->addField('report_type', 'select', array(
            'name'      => 'report_type',
            'options'   => $this->_getReportTypeOptions(),
			'onchange'	=>"getCurrencyCode('$adminurl'+this.value)",
            'label'     => Mage::helper('queryreport')->__('Select Existing Report'),
			 'note' => $this->__("&nbsp;")
        ));
		
		
       $fieldset->addField('description_type', 'text', array(
            'name'      => 'description_type',
            //'options'   => $this->_getPeriodTypeOptions(),
            'label'     => Mage::helper('queryreport')->__('Description')
        ));
		
		$fieldset->addField('save_report', 'select', array(
            'name'      => 'save_report',
            'options'   => $this->_getReportSaveReportOption(),
            'label'     => Mage::helper('queryreport')->__('Do you want to save or update report')
        ));
		
		
        
		
		 $fieldset1->addField('query_type', 'textarea', array(
            'name'      => 'query_type',
            //'options'   => $this->_getPeriodTypeOptions(),
			'value'     =>'',  
            'label'     => Mage::helper('queryreport')->__('Sql query box') 
  ...
  $form->setUseContainer(true);
    ...
    protected function _getReportTypeOptions()
    {
		$collectionReport=Mage::getModel('queryreport/reportsave')->getCollection();
		$options=array();
		$options['novalue']= '';
		foreach($collectionReport as $data)
		{
			$options[$data->getId()]= $data->getDescriptionType();
			
			}
		
  /*    $options        = array(
            'genral_order'       => Mage::helper('queryreport')->__('General Order'),
         'latest_order'     => Mage::helper('queryreport')->__('Latest Order'),
           'order_export'      => Mage::helper('queryreport')->__('Order Export'),
			'list'      => Mage::helper('queryreport')->__('List Items of never sold'),
        );*/

        return $options;
    }
	
    protected function _getReportSaveReportOption()
    {
        $options        = array(
            'no'       => Mage::helper('queryreport')->__('No'),
            'yes'     => Mage::helper('queryreport')->__('Yes'),         
        );
