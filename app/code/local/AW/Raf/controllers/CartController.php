...
class AW_Raf_CartController extends Mage_Core_Controller_Front_Action
{
    public function createCouponAction()
    {
        ...
        $amount = (float) preg_replace("#[^.,0-9]#isu", "", $post['awraf-amount']);
        ...
        if ($amount) {
            ...
            ;
            $baseMargin = (float)round(
                $availableAmount - Mage::helper('awraf')->getAppliedAmount(),
                AW_Raf_Helper_Data::PRECISION
            );

            $margin = Mage::helper('awraf')->convertAmountByCurrencyCode(
                $baseMargin,
                $store->getBaseCurrency()->getCode(),
                $store->getCurrentCurrency()->getCode()
            );

            if (floatval($amount) <= round(floatval($margin), AW_Raf_Helper_Data::CURRENCY_DECIMALS)) {
                $baseAmount = Mage::helper('awraf')->convertAmountByCurrencyCode(
                    $amount,
                    $store->getBaseCurrency()->getCode(),
                    $store->getCurrentCurrency()->getCode(),
                    array(
                ...
                Mage::helper('awraf')->setReservedAmount($baseAmount);
                $quote = Mage::helper('awraf/quote')->getQuote();
                $quote->getShippingAddress()->setCollectShippingRates(true);
                $quote
                    ->collectTotals()
