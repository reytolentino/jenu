    ...
    public function onepageCheckoutSaveOrderBefore($observer)
    {
        ...
        if (!Mage::getSingleton('sarp/subscription')->getId() && $haveSarpItems) {
            switch($order->getPayment()->getMethod()) {
                ...
                break;
                case AW_Sarp_Model_Payment_Method_Authorizenetcim::PAYMENT_METHOD_CODE:
                    // CIM Account should be created by another module
                    // Gorill/AuthorizenetCim
                break;
                default:
            }
        } elseif ($haveSarpItems) {
            switch($order->getPayment()->getMethod()) {
                case AW_Sarp_Model_Payment_Method_Authorizenetcim::PAYMENT_METHOD_CODE:
                    $paymentModel = Mage::getModel('sarp/payment_method_authorizenetcim');
                    $qPayment = $quote->getPayment();
                    $qPayment->setAdditionalInformation('authorizenetcim_customer_id', $qPayment->getAuthorizenetcimCustomerId());
                    $qPayment->setAdditionalInformation('authorizenetcim_payment_id', $qPayment->getAuthorizenetcimPaymentId());
                    $oPayment = $order->getPayment();
                    $oPayment->setAdditionalInformation('authorizenetcim_customer_id', $oPayment->getAuthorizenetcimCustomerId());
                    $oPayment->setAdditionalInformation('authorizenetcim_payment_id', $oPayment->getAuthorizenetcimPaymentId());
                    $service = $paymentModel->getWebService();
                    $service->setPayment($qPayment);
                    $subscription = new Varien_Object();
                    $subscription->setStoreId($order->getStoreId());
                    $service->setSubscription($subscription);
                    try{
                        //$CIMInfo = $service->loadCIMAccount($service->getPayment()->getAuthorizenetcimCustomerId());
                        $additionalInformation = array(
                            'authorizenetcim_customer_id' => $service->getPayment()->getAuthorizenetcimCustomerId(),
                            'authorizenetcim_payment_id' => $service->getPayment()->getAuthorizenetcimPaymentId()
                        );
                        $service->getPayment()->setAdditionalInformation($additionalInformation);
                    }
                    catch(Exception $e){
                        throw new Mage_Core_Exception($e->getMessage());
                    }
