        ...
        if (!Mage::getModel('sarp/product_type_default')->validateSubscription($product, $buyRequest)) {
            ...
            if ($buyRequest->getAwSarpSubscriptionStart() && $Period->getId()) {
                ...
                if ($performDateCompare
                    ...
                    $tempProduct->setDescription(null);
                    $tempProduct->setMediaGallery(null);
                    $tempProduct->setShortDescription(null);
                    $tempProduct->setCustomLayoutUpdate(null);
                    $tempProduct->setBenefits(null);
                    $tempProduct->setGuarantee(null);
                    $tempProduct->setIngredients(null);

                    Mage::log(array(
                            'date' => $date->toString('M/d/yy'),
                            'today' => $today->toString("M/d/yy"),
                            'weekday' => $date->get(Zend_Date::WEEKDAY_DIGIT),
                            'performDateCompare' => $performDateCompare,
                            'date_compare' => $date->compare($today, Zend_Date::DATE_SHORT),
                            'date->compare($today, Zend_Date::DATE_SHORT)' => $date->compare($today, Zend_Date::DATE_SHORT),
                            '!Period->isAllowedDate' => !$Period->isAllowedDate($date, $product),
                            'period' => $Period->getData(),
                            'buyRequest' => $buyRequest->debug(),
                            'product' => $tempProduct->debug()
                        ),
                        null,
                        "subscription_products.log"
                    );
                    unset($tempProduct);
