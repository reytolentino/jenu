...
class AW_Sarp_Model_Subscription extends AW_Core_Model_Abstract
{
    ...
    public function getLoggedIssues() {
        if($this->getStatus() != self::STATUS_SUSPENDED AND !$this->isActiveWithErrors()) {
            return array();
        }
        $loggers = Mage::getModel('awcore/logger')->getCollection();
        $select = $loggers->getSelect();
        $loggers->setOrder('date', 'DESC');
        $select->where('`title` LIKE ? OR `content` LIKE ?', '%#'.$this->getId().'%');
        if(! $loggers->count()) {
            return array();
        }
        $ret = array();
        foreach($loggers AS $logger) {
            if(self::contains($logger->getTitle().$logger->getContent(), $this->getId()))
                $ret[] = $logger;
        }
        return $ret;
    }

    protected static function contains($string, $subscriptionId) {
        preg_match_all('/#\d+/', $string.$string.$string.$string, $m);
        if(! empty($m[0])) {
            foreach($m[0] AS $n) {
                if($n == '#'.$subscriptionId)
                    return TRUE;
    ...
    /**
     * Check if subscription is active and has errors
     */
    public function isActiveWithErrors(){
        // Check active status
        if($this->getStatus() != self::STATUS_ENABLED) {
            return FALSE;
        }
        // Get sequence with error
        $coll = Mage::getModel('sarp/sequence')
            ->getCollection()
            ->addSubscriptionFilter($this)
            ->addStatusFilter(AW_Sarp_Model_Sequence::STATUS_FAILED)
    ...
    public function clearErrorForLastSequence() {
        if(! $this->isActiveWithErrors()) {
            return FALSE;
        }
        // Get last sequence with error
        $coll = Mage::getModel('sarp/sequence')
            ->getCollection()
            ->addSubscriptionFilter($this)
            ->addStatusFilter(AW_Sarp_Model_Sequence::STATUS_FAILED)
            ->setOrder('date', Varien_Data_Collection::SORT_ORDER_DESC)
        ;
        $sequence = $coll->getFirstItem();
        if($sequence->getId()) {
            try {
                $sequence->setStatus(AW_Sarp_Model_Sequence::STATUS_PENDING);
                $sequence->save();
                return TRUE;
            }
            catch(Exception $e) {
