...
class AW_Sarp_Model_Web_Service_Client_Authorizenetcim extends AW_Sarp_Model_Web_Service_Client
{
    ...
    public function getApiLoginId()
    {
        ...
        $node = AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_TEST_MODE 
              ? AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_API_TEST_LOGIN_ID 
              : AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_API_LOGIN_ID;
        return Mage::getStoreConfig($node, $_storeId);
    }
    ...
    public function getTransactionKey()
    {
        ...
        if($this->getSubscription()->getStoreId())
            $_storeId = $this->getSubscription()->getStoreId();
        $node = AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_TEST_MODE 
              ? AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_TEST_TRANSACTION_KEY 
              : AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_TRANSACTION_KEY;
        return Mage::getStoreConfig($node, $_storeId);
    }

    /**
     * Capture or auth only
     * @return Mage_Paygate_Model_Authorizenetcim::ACTION_AUTHORIZE || Mage_Paygate_Model_Authorizenetcim::ACTION_AUTHORIZE_CAPTURE
     */
    public function getPaymentAction()
    {
        return Mage::getStoreConfig(AW_Sarp_Model_Payment_Method_Authorizenetcim::XML_PATH_AUTHORIZENETCIM_PAYMENT_ACTION);
    }
    ...
    public function createCIMAccount()
    {
        ...
        if ($result = $this->_runRequest('CreateCustomerProfile', array(self::ERR_CODE_DUPLICATE_CUSTOMER))) {
            ...
            if ($result instanceof AW_Sarp_Model_Web_Service_Client_Authorizenetcim_Error) {
    ...
    public function loadCIMAccount($customer_profile_id)
    {
        $this->getRequest()
                ->reset()
                ->setData(array(
        ...
        $result = $this->_runRequest('GetCustomerProfile'); // $customer_payment_profile_id = $result->profile->paymentProfiles->CustomerPaymentProfileMaskedType->customerPaymentProfileId;
        return $result;
    }
    ...
    {
        if ($this->getPaymentAction() == Mage_Paygate_Model_Authorizenetcim::ACTION_AUTHORIZE_CAPTURE) {
     ...
     * @return AW_Sarp_Model_Web_Service_Client_Authorizenetcim
     */
    ...
    {
        $obj = new AW_Sarp_Model_Web_Service_Client_Authorizenetcim_Error;
