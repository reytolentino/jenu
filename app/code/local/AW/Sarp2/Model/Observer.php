...
class AW_Sarp2_Model_Observer
{
    ...
    public function salesQuoteAddItem($observer)
    {
        ...
        if ($subscriptionTypeOption && !is_null($subscriptionStartDateOption)) {
            try {
    ...
    public function catalogControllerProductView(Varien_Event_Observer $observer)
    {
        ...
        $helper->addSubscriptionTypeSelectorToSubscription($subscription, $product, 'Select period');
        $helper->addSubscriptionStartDateOptionToSubscription($subscription, $product, 'Start date');
