...
class Eternal_Ajaxfilter_Block_Catalog_Layer_Filter_Price extends Mage_Catalog_Block_Layer_Filter_Price 
{
	...
	*/
	public function __construct(){
		...
		$this->_imagePath = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'wysiwyg/eternal/ajaxfilter/';
	...
	*/
	...
	public function getPriceDisplayType(){
		...
		if($this->isTextBoxEnabled()){
			$html = '
				<div class="text-box">
					'.$this->getCurrencySymbol().' <input type="text" name="min" id="minPrice" class="priceTextBox minPrice" value="'.$this->getCurrMinPrice().'" /> - 
					'.$this->getCurrencySymbol().' <input type="text" name="max" id="maxPrice" class="priceTextBox maxPrice" value="'.$this->getCurrMaxPrice().'" />
                    <a href="#" class="button filter-button">'.$this->getGoBtnText().'</a>
			...
			$html = '<div class="clearfix"><p class="price-range"><label>'.$this->__('Price: ').'</label>
					<input type="text" id="amount" class="price-amount" readonly="readonly" style="background:none; border:none;" value="'.$this->getCurrencySymbol().$this->getCurrMinPrice()." - ".$this->getCurrencySymbol().$this->getCurrMaxPrice().'" /></p>
                    <p class="filter-btn"><a href="#" class="button filter-button">'.$this->getGoBtnText().'</a>
					</p></div>';	
		}
		return $html;
	}
		...
		$html = '
			<script type="text/javascript">
				jQuery(function($) {
                    ...
                    if ($this->isTextBoxEnabled()) $html .= 'url = getUrl($(".minPrice").val(), $(".maxPrice").val());';
                    $html .= $ajaxCall.'	
					});
	...
	*/
	...
	public function prepareCustomStyles(){
		...
		if($useImage){
			...
			if($handle){$bgHandle = 'url('.$this->_imagePath.$handle.') no-repeat';} else {$bgHandle = $this->getConfig('price_slider/price_slider_conf/handle_color');}
			if($range){$bgRange = 'url('.$this->_imagePath.$range.') no-repeat';} else {$bgRange = $this->getConfig('price_slider/price_slider_conf/range_color');}
			if($slider){$bgSlider = 'url('.$this->_imagePath.$slider.') no-repeat';} else {$bgSlider = $this->getConfig('price_slider/price_slider_conf/background_color');    }
