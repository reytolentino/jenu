...
class Smartwave_Blog_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    const XML_BLOG_TOP_MENU = 'blog/blog/top_menu';
    /* menus and links */
    ...
    public function includeMenu($store = null)
    {
        return $this->conf(self::XML_BLOG_TOP_MENU, $store);
    }
    
    public function getMenuWideLink()
    {
        $html = array();
        if ($this->getEnabled()) {
            $html[] = '<div id="menu_blog" class="menu">';
            $html[] = '<div class="parentMenu">';
            $html[] = '<a href="'.Mage::getUrl($this->getRoute()).'">';
            $html[] = '<span>'.$this->__($this->getTitle()).'</span>';
            $html[] = '</a>';
            $html[] = '</div>';
            $html[] = '</div>';
            $html = implode("\n", $html);
    ...
    public function getMenuNarrowLink()
    {
        $html = array();
        if ($this->getEnabled()) {
    ...
    public function getMobileMenuLink()
    {
        $html = array();
        if ($this->getEnabled()) {
            $html[] = '<div id="menu-mobile-35" class="menu-mobile level0"><div class="parentMenu">';
            $html[] =  '<a href="'.Mage::getUrl($this->getRoute()).'">';
            $html[] = '<span>'.$this->__($this->getTitle()).'</span>';
            $html[] = '</a></div></div>';
            $html = implode("\n", $html);
