 ...
 * ID:            %!uniqueid!%
 * Packaged:      %!packaged!%
 * Last Modified: 2015-04-16T20:38:47+02:00
...
class Xtento_OrderStatusImport_Model_Processor_Csv
{
    ...
    public function getRowsToProcess($filesToProcess)
    {
        ...
        /*
        $bkp = $this->config['IMPORT_DELIMITER'];
        $bkp_header = $this->config['IMPORT_SKIP_HEADER'];
        $bkp_order = $this->mapping->getData('order_number');
        $bkp_track = $this->mapping->getData('tracking_number');
        ...
        $isSecondProfile = false;
        if (!preg_match("/UPS/mis", $filename)) {
        // Profile 2 active
        $isSecondProfile = true;
        $this->config['IMPORT_SKIP_HEADER'] = true;
        $this->config['IMPORT_DELIMITER'] = ";";
        $this->mapping->setData('order_number', 2);
        $this->mapping->setData('tracking_number', 19);
        } else {
        $this->config['IMPORT_SKIP_HEADER'] = $bkp_header;
        $this->config['IMPORT_DELIMITER'] = $bkp;
        $this->mapping->setData('order_number', $bkp_order);
        $this->mapping->setData('tracking_number', $bkp_track);
        }
         */
